...
def create_collate_fn(augmenter: Callable[[torch.Tensor, Optional[int]], torch.Tensor],
    ...
    def collate_fn(input_batch: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        # Anchors
        ...
        random_length = np.random.randint(*(np.minimum(audio_length_range, [min_length] * 2) + np.array([0, 1])))
